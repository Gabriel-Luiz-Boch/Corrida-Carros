...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['cadastro_inicial'])) {
            ...
            // Insere os dados do carro no banco de dados
            ...
            // Verifica se o carro já está cadastrado na prova
            ...
            if (!$naoParticipou) {
                ...
                // Recupera a lista de carros e suas distâncias percorridas, ordenada pela distância
                ...
                // Atualiza a colocação e pontuação de acordo com a distância percorrida
                ...
                // Se o carro não participou, insere os dados com pontuação zero
            ...
            if (!$naoParticipou) {
                ...
                // Recupera a lista de carros e seus tempos ajustados, ordenada pelo tempo ajustado
                ...
                // Atualiza a colocação e pontuação de acordo com o tempo ajustado
                while ($row = $stmt->fetch()) {
                    $posicaoAtual++;
            ...
            if (!$naoParticipou) {
                ...
                // Recupera a lista de carros e seus pesos retentores, ordenada pelo peso
                ...
                // Atualiza a colocação e pontuação de acordo com o peso retentor
                while ($row = $stmt->fetch()) {
                    $posicaoAtual++;
...
<head>
    ...
    <!-- Inclui CSS do DataTables -->
    <link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/1.10.24/css/jquery.dataTables.css">
    <!-- Inclui CSS do Toastr -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.css">
    <style>
        .container {
            max-width: 1300px; /* Aumentar largura do container */
        }
    </style>
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <a class="navbar-brand" href="#" onclick="mostrarCadastro('cadastro')"><b>COMPETIÇÃO DE CARROS</b></a>
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item">
                    <button type="button" class="btn btn-danger"><a class="nav-link" href="logout.php"><b>Sair</b></a></button>
                </li>
            </ul>
        </div>
    </nav>
        ...
        <div class="mt-4">
            ...
            <table id="tabela-carros-cadastrados" class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo "<tr onclick=\"mostrarInformacoesEquipe({$row['id']})\">";
    ...
    <div class="container mt-4" id="prova1" style="display: none;">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            ...
            <button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#editModal" onclick="setupEditModal('prova1')">Editar</button>
        </form>
    ...
    <div class="container mt-4" id="prova2" style="display: none;">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            ...
            <button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#editModal" onclick="setupEditModal('prova2')">Editar</button>
        </form>
    ...
    <div class="container mt-4" id="prova3" style="display: none;">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            ...
            <button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#editModal" onclick="setupEditModal('prova3')">Editar</button>
        </form>
        <hr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editForm">
                        <div class="form-group">
                            <label for="editCarroId">Número do Carro</label>
                            <select class="form-control" id="editCarroId" name="editCarroId">
                                <option selected>Selecione o nº do carro</option>
                                <?php
                                $stmt = $conn->query("SELECT id, numero FROM carros");
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    <!-- Modal Informações da Equipe -->
    <div class="modal fade" id="informacoesEquipeModal" tabindex="-1" role="dialog" aria-labelledby="informacoesEquipeModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="informacoesEquipeModalLabel">Informações da Equipe:</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <h4 id="equipeTitulo"></h4>
    ...
    <script>
        ...
        function setupEditModal(prova) {
            ...
            if (prova === 'prova1') {
                document.getElementById('editFields').innerHTML = `
                    <div class="form-group">
                        <label for="editDistanciaPercorrida">Distância Percorrida</label>
                        <input type="text" class="form-control" id="editDistanciaPercorrida" name="editDistanciaPercorrida">
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="editNaoParticipou" name="editNaoParticipou">
                        <label class="form-check-label" for="editNaoParticipou">
                            Não participou
                        </label>
                    </div>
                    <input type="hidden" name="formType" value="prova1">
                `;
            } else if (prova === 'prova2') {
                document.getElementById('editFields').innerHTML = `
                    <div class="form-group">
                        <label for="editTempo">Tempo (segundos)</label>
                        <input type="text" class="form-control" id="editTempo" name="editTempo">
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="editPenalidade" name="editPenalidade">
                        <label class="form-check-label" for="editPenalidade">
                            Penalidade
                        </label>
                    </div>
                    <div class="form-check">
                    ...
                    <input type="hidden" name="formType" value="prova2">
                `;
            } else if (prova === 'prova3') {
                document.getElementById('editFields').innerHTML = `
                    <div class="form-group">
                        <label for="editPesoRetentor">Peso Retentor (kg)</label>
                        <input type="text" class="form-control" id="editPesoRetentor" name="editPesoRetentor">
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="editNaoParticipou" name="editNaoParticipou">
                        <label class="form-check-label" for="editNaoParticipou">
                    ...
                    <input type="hidden" name="formType" value="prova3">
                `;
            }
        }
        ...
        document.getElementById('editForm').addEventListener('submit', function (e) {
            ...
            .then(data => {
                if (data.success) {
                    // Atualiza a página ou atualiza a tabela com os novos dados
                    location.reload();
                } else {
    ...
    <script>
        function mostrarInformacoesEquipe(equipeId) {
            $.ajax({
                url: 'info_equipe.php',
                type: 'POST',
                ...
                success: function(response) {
                    ...
                    $('#equipeTitulo').text(dados.numero + ' - ' + dados.equipe);
                    $('#listaIntegrantes').empty();
                    dados.integrantes.forEach(function(integrante) {
                        $('#listaIntegrantes').append('<li>' + integrante + '</li>');
                    });
                    $('#informacoesEquipeModal').modal('show');
                },
                error: function() {
                    alert('Erro ao obter informações da equipe.');
                }
            });
        }
    </script>
