...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        elseif (isset($_POST['prova_2'])) {
            ...
            $numPenalidades = filter_input(INPUT_POST, 'numPenalidades', FILTER_VALIDATE_INT) ?: 0;
            $naoParticipou = isset($_POST['nao_participou']) ? 1 : 0;
            ...
            if (!$naoParticipou) {
                $tempoAjustado = $tempo + ($penalidade ? 2 * $numPenalidades : 0);
...
<body>
    ...
    <!-- Conteúdo da prova 2 -->
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            ...
            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="penalidade" name="penalidade" onchange="togglePenalidade()">
                <label class="form-check-label" for="penalidade">
                    Penalidade
                </label>
            </div>
            <div class="form-group" id="numPenalidadesGroup" style="display: none;">
                <label for="numPenalidades">Número de Penalidades</label>
                <input type="number" class="form-control" id="numPenalidades" name="numPenalidades" min="0" placeholder="Número de penalidades">
            </div>
            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="nao_participou" name="nao_participou">
                <label class="form-check-label" for="nao_participou">
    ...
    <div class="container mt-4" id="resultado" style="display: none;">
        ...
        <table id="resultadosFinais" class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                <?php
                // Query para recuperar os dados dos carros e suas pontuações
                $stmt = $conn->query('
                    SELECT carros.numero, carros.equipe, 
                        ...
                        (IFNULL((SELECT MIN(colocacao) FROM prova_1 WHERE prova_1.carro_id = carros.id), 0) +
                            IFNULL((SELECT MIN(colocacao) FROM prova_2 WHERE prova_2.carro_id = carros.id), 0) +
                            IFNULL((SELECT MIN(colocacao) FROM prova_3 WHERE prova_3.carro_id = carros.id), 0)) / 
                        (CASE WHEN (SELECT COUNT(*) FROM prova_1 WHERE prova_1.carro_id = carros.id) = 1 THEN 1 ELSE 0 END +
                            CASE WHEN (SELECT COUNT(*) FROM prova_2 WHERE prova_2.carro_id = carros.id) = 1 THEN 1 ELSE 0 END +
                            CASE WHEN (SELECT COUNT(*) FROM prova_3 WHERE prova_3.carro_id = carros.id) = 1 THEN 1 ELSE 0 END) AS colocacao_media
                    FROM carros
                    ORDER BY colocacao_media ASC, pontuacao_total DESC
                ');
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo '<td>' . $row['equipe'] . '</td>';
                    echo '<td>' . number_format($row['pontuacao_total'], 1) . '</td>';
                    echo '<td>' . number_format($row['colocacao_media'], 2) . '</td>';
                    echo '<td>' . $posicao . '</td>';
                    echo '</tr>';
                    $posicao++;
                }
    ...
    $(document).ready(function () {
        $('#resultadosFinais').DataTable({
            "order": [[3, "asc"]], // Ordena pela 4ª coluna (colocação média) em ordem ascendente
            "language": {
                "lengthMenu": "Mostrar _MENU_ resultados", // Texto personalizado para mostrar a quantidade de resultados
                "info": "Mostrando _START_ a _END_ de _TOTAL_ resultados", // Texto personalizado para mostrar informações sobre a exibição de resultados
                "search": "Pesquisar:", // Texto personalizado para a barra de pesquisa
    ...
    <script>
        ...
        // Função para mostrar/ocultar o campo de número de penalidades
        function togglePenalidade() {
            var penalidadeCheckbox = document.getElementById('penalidade');
            var numPenalidadesGroup = document.getElementById('numPenalidadesGroup');
            if (penalidadeCheckbox.checked) {
                numPenalidadesGroup.style.display = 'block';
            } else {
                numPenalidadesGroup.style.display = 'none';
            }
        }
    </script>
